...
module znd
	...
	contains
	  ...
	  ! Needed to add the null_solout as mesa-26.04.1/num/pulic/num_lib.f90 has private and they did not add
	  ! public :: null_solout
	  ! therefore to procced without editing the MESA build I added it here 
	  ! this is the same subroutine from the file 
	  ...
	  ! copied this from mesa-26.04.1/num/public/num_newton.dek as it is not being passed out of the num_lib.f90 file 
...
!         		dfdx(species+5, species+4) = 1d0/ux
    ...
    subroutine znd_integrate(x_start, x_end, num_steps, output_profile, profile_io, find_lburn)
    	...
    	double precision :: log_t_min, log_t_max, dlog		!New Integration bounds
...
!        	!	t_neumann, burn_u, burn_time, vars, rpar(1)
      ...
      subroutine cleanup
        ...
        if (ierr /= 0) STOP "*** Deallocation error 2 ***"
      	deallocate(which_rates, rtol, atol, stat=ierr)
